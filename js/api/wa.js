...
// (simpan/hapus), & helper memori ALL_TUGAS / ALL_SCHEDULES — domain
...
// menimpa di memori + render ulang dari memori (renderTugas/renderJadwal
...
function upsertScheduleMemory(s) {
...
function removeScheduleMemory(id) {
  if (!id) return;
...
async function tambahTugasAdmin() {
  ...
  try {
    ...
    if (!res.success) {
      input.value = text;
      showToast(res.error || tr('alert.failed'), 'error');
    } else {
...
async function submitJadwal(e) {
  ...
  try {
    ...
    btn.innerHTML = tr('button.save_schedule');
    btn.disabled = false;
