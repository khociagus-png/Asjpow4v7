    ...
    var RB_PRESETS = {
        include: ['TIKET PESAWAT', 'VISA (SUBSIDI 1JT)', 'ASURANSI KESEHATAN', 'TRAINING BAHASA JEPANG 3 BULAN', 'SURAT-SURAT ADMINISTRASI'],
        exclude: ['PASPOR', 'MCU (MEDICAL CHECK UP)', 'EKTLN', 'AKOMODASI SEHARI-HARI', 'MAKAN SEHARI-HARI'],
        benefit: ['PENEMPATAN KAISHA/KLINIK', 'GAJI POKOK 180.000 YEN UP', 'BANTUAN GAJI: LEMBUR & KENIKMATAN LAINNYA', 'SUBSIDI GAJI / KUOTA BIAYA KOKO', 'KAIGO PASAL 2 JOMPO'],
        persyaratan: ['MENGIRIM CV SESUAI FORMAT JOB TERKAIT', 'MENGIRIM SERTIFIKAT JLPT/JFT & SSW ATAU SENMONKYU (1 FILE PDF)', 'USIA 18-28 TAHUN', 'PUNYA PENGALAMAN KERJA DI INDONESIA', 'MENGIKUTI TRAINING BAHASA JEPANG (BELUM LULUS JFT/JLPT)']
    };
    ...
    var RB_DEFAULT_CATATAN = 'APABILA PERUSAHAAN (KAISHA) MEMBATALKAN PROSES KEBERANGKATAN, BIAYA YANG TELAH DIBAYARKAN AKAN DIKEMBALIKAN SESUAI KETENTUAN YANG BERLAKU.';
    ...
    function rbResetTahapan() {
        ...
        RB_DEFAULT_TAHAPAN.forEach(function (t) { rbAddTahapan(t.nama, t.nominal); });
        rbRenderPreview();
    }

    function rbEl(id) { return document.getElementById(id); }
    function rbQsa(sel) { return Array.prototype.slice.call(document.querySelectorAll(sel)); }
    ...
    function rbChipEl(sec, val, dbId) {
        ...
        chip.onclick = function () { rbToggleChip(chip); };
    ...
    async function rbLoadPresetsFromDb(cb) {
        if (typeof callAPI !== 'function') { if (cb) cb(false); return; }
        try {
    ...
    function rbSyncChipStars(sec) {
        ...
        rbQsa('#rb-' + sec + ' .rb-chip').forEach(function (chip) {
            ...
            var found = RB_DB_PRESETS[sec].some(function (p) { return String(p.item).trim().toUpperCase() === val; });
            var star = chip.querySelector('.rb-star');
            if (!star) return;
            if (found && !chip.getAttribute('data-rb-db-id')) {
                chip.setAttribute('data-rb-db-id', String(RB_DB_PRESETS[sec].filter(function (p) { return String(p.item).trim().toUpperCase() === val; })[0].id));
            }
            if (found) { star.classList.add('rb-star-saved'); star.innerHTML = '<i class="fas fa-star"></i>'; star.title = tr('ui.remove_fav'); }
            else { star.classList.remove('rb-star-saved'); star.innerHTML = '<i class="far fa-star"></i>'; star.title = tr('ui.add_fav'); }
        });
    }
    ...
    async function rbSavePreset(sec, val, chip, star) {
        ...
        try {
            ...
            if (res && res.success) {
                ...
                star.classList.add('rb-star-saved'); star.innerHTML = '<i class="fas fa-star"></i>'; star.title = tr('ui.remove_fav');
    ...
    async function rbUnsavePreset(sec, dbId, chip, star) {
        ...
        try {
            ...
            if (res && res.success) {
                ...
                star.classList.remove('rb-star-saved'); star.innerHTML = '<i class="far fa-star"></i>'; star.title = tr('ui.add_fav');
    ...
    function rbAddTahapan(nama, nominal) {
        ...
        row.innerHTML = '<input type="text" class="rb-tahapan-nama flex-1 p-2 rounded-lg bg-black/60 border border-slate-700 text-white text-xs outline-none focus:border-amber-500 transition" placeholder="' + tr('ui.stage_name_ph') + '" value="' + rbAttr(nama) + '">' +
            '<input type="text" class="rb-tahapan-nominal w-20 p-2 rounded-lg bg-black/60 border border-slate-700 text-emerald-300 text-xs font-bold outline-none focus:border-amber-500 transition" placeholder="' + tr('ui.stage_nominal_ph') + '" value="' + rbAttr(nominal) + '">' +
            '<button type="button" onclick="rbRemoveTahapan(this)" aria-label="' + tr('ui.delete_stage') + '" class="w-8 h-8 flex items-center justify-center bg-red-900/40 hover:bg-red-600 text-red-300 hover:text-white rounded-lg text-xs font-bold transition flex-shrink-0" title="' + tr('ui.delete_stage') + '"><i class="fas fa-times"></i></button>';
    ...
    function rbRemoveTahapan(btn) {
        ...
        if (row) { row.remove(); rbRenderPreview(); }
    }
    ...
    function rbSerialize() {
        ...
        if (steps.length) { out.push('TAHAPAN PEMBAYARAN'); steps.forEach(function (s) { out.push(s); }); out.push(''); }
        ['include', 'exclude', 'benefit', 'persyaratan'].forEach(function (sec) {
            var items = [];
            // textContent chip menyertakan ikon bintang (★/☆) - buang sebelum
            // diserialkan supaya teks rincian tidak tercemar karakter bintang.
            rbQsa('#rb-' + sec + ' .rb-chip.rb-on').forEach(function (c) { items.push(c.textContent.replace(/[★☆]/g, '').trim()); });
            if (items.length) { out.push(sec.toUpperCase()); items.forEach(function (x) { out.push('• ' + x); }); out.push(''); }
        });
        var catatan = (rbEl('rb-catatan').value || '').trim();
        if (catatan) { out.push('CATATAN'); catatan.split('\n').forEach(function (l) { out.push(l); }); }
        return out.join('\n').replace(/\n{3,}/g, '\n\n').trim();
    }
    ...
    function rbRenderPresets() {
        ['include', 'exclude', 'benefit', 'persyaratan'].forEach(function (sec) {
            ...
            var items = (list && list.length) ? list.map(function (p) { return { item: p.item, id: p.id }; })
                : (RB_PRESETS[sec] || []).map(function (v) { return { item: v, id: null }; });
    ...
    function rbSeedFromText(text) {
        var parsed = (typeof parseRincianBiaya === 'function') ? parseRincianBiaya(text || '') : { total: '', sections: [] };
        ...
        ['include', 'exclude', 'benefit', 'persyaratan'].forEach(function (sec) {
            ...
            rbQsa('#rb-' + sec + ' .rb-chip').forEach(function (c) {
                ...
                existing.push(v);
                if (items.some(function (x) { return String(x).trim().toUpperCase() === v; })) c.classList.add('rb-on');
            });
            items.forEach(function (x) {
                var v = String(x).trim();
                if (!v) return;
                if (existing.indexOf(v.toUpperCase()) < 0) { rbEl('rb-' + sec).appendChild(rbChipEl(sec, v)); }
            });
        });
    ...
    function rbSummaryFromData(total, rincian) {
        var parsed = (typeof parseRincianBiaya === 'function') ? parseRincianBiaya(rincian || '') : { total: '', sections: [] };
        var p = parsed || { total: '', sections: [] };
        var t = total || p.total || '';
        var n = function (type) { return rbSectionItems(p.sections, type).length; };
        var parts = [];
    ...
    function openRincianBuilder(target) {
        ...
        function finalize() {
            ...
            ['include', 'exclude', 'benefit', 'persyaratan'].forEach(function (s) { rbSyncChipStars(s); });
            rbRenderPreview();
    ...
    function rbSummaryHtml() {
        ...
        var steps = rbQsa('#rb-tahapan-list .rb-tahapan-row').filter(function (r) { return (r.querySelector('.rb-tahapan-nama').value || '').trim(); }).length;
        var cnt = function (sec) { return rbQsa('#rb-' + sec + ' .rb-chip.rb-on').length; };
        var parts = [];
        if (t) parts.push('💵 Total ' + t);
