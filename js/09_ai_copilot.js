    ...
    async function kirimPesanAdminAi(event) {
        ...
        chatBox.insertAdjacentHTML('beforeend', '<div id="' + typingId + '" class="flex items-start gap-3 mt-4 fade-in">' +
            '<img src="' + urlFotoJeklin + '" class="w-8 h-8 rounded-full object-cover shadow-sm border border-amber-400 flex-shrink-0" alt="Jeklin" onerror="this.style.display=\'none\'">' +
    ...
    function autoFillFormDariAi(data) {
        ...
        if (data.gender) {
            if (data.gender.id_indonesia) document.getElementById('ai-k-gender-id').value = data.gender.id_indonesia;
            if (data.gender.id_jepang) document.getElementById('ai-k-gender-jp').value = data.gender.id_jepang;
        }
        
        if (data.status_nikah) {
            if (data.status_nikah.id_indonesia) document.getElementById('ai-k-nikah-id').value = data.status_nikah.id_indonesia;
            if (data.status_nikah.id_jepang) document.getElementById('ai-k-nikah-jp').value = data.status_nikah.id_jepang;
        }
    ...
    function tambahPesanAdminAi(text, sender) {
        ...
        let htmlStr = '';
        if (isUser) {
            htmlStr = '<div class="flex justify-end gap-3 mt-4 fade-in">' +
                '<div class="bg-sky-600 text-white text-sm p-3.5 rounded-2xl rounded-tr-none shadow-md" style="max-width: 85%; width: fit-content;">' +
                    '<p class="whitespace-pre-wrap m-0 leading-relaxed">' + cleanText + '</p>' +
                '</div>' +
            '</div>';
        } else {
            htmlStr = '<div class="flex items-start gap-3 mt-4 fade-in">' +
                '<img src="' + urlFotoJeklin + '" class="w-8 h-8 rounded-full object-cover shadow-sm border border-amber-400 flex-shrink-0" alt="Jeklin" onerror="this.style.display=\'none\'">' +
                '<div class="bg-slate-800 text-slate-200 text-sm p-3.5 rounded-2xl rounded-tl-none shadow-md border border-amber-500/20" style="max-width: 85%; width: fit-content;">' +
                    '<p class="whitespace-pre-wrap m-0 leading-relaxed">' + cleanText + '</p>' +
                '</div>' +
    ...
    function tampilkanSaranAdminAi(actions) {
        ...
        actions.forEach(act => {
            ...
            btn.className = 'whitespace-nowrap px-4 py-2 bg-slate-800 hover:bg-slate-700 text-amber-400 text-xs rounded-full transition-colors font-medium border border-slate-700 flex-shrink-0 shadow-sm';
            btn.innerText = act;
            btn.onclick = () => {
    ...
    function appendInterviewChat(sender, text) {
        ...
        if (isUser) {
            ...
            htmlStr = '<div class="flex items-start gap-3 mt-4 fade-in">' +
                '<img src="' + urlFotoJeklin + '" class="w-8 h-8 rounded-full object-cover shadow-sm border border-violet-400 flex-shrink-0" alt="Jeklin" onerror="this.style.display=\'none\'">' +
