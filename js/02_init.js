    ...
    // ==========================================
    var ALL_JOBS = []; var ALL_DB_JOBS = []; var ALL_CANDIDATES = []; var ALL_SCHEDULES = []; 
    var ALL_TUGAS = []; var ALL_FORM = []; var ALL_WA_TEMPLATES = []; var ALL_RIWAYAT_KANDIDAT = [];
    var ASSETS = {}; var CURRENT_THEME = 'TOKYO'; var DROPDOWNS = {};
    var isAdmin = false; var isKandidat = false; var currentAdminName = ''; var currentKandidatName = ''; var currentKandidatWa = ''; var currentKandidatId = '';
    var limitPub = 10, limitAdm = 10, limitKan = 10, limitJad = 10, limitDb = 10; 
    var dbSortType = 'TERBARU', dbFilterBidang = 'ALL', dbFilterTahapan = 'ALL';
    var mailFilterStatus = 'MENUNGGU'; var mailSearchText = '';
    var currentPublicFilter = 'ALL'; var currentCopyListTxt = '';
    ...
    var THEMES = { 
        SAKURA: { bg: 'bg-gradient-to-b from-[#bda8ae] via-[#cbb4bb] to-[#cbb4bb] text-stone-900', border: 'border-rose-400/60', head: 'bg-pink-800 text-white' }, 
        TOKYO: { bg: 'bg-slate-950 text-slate-100', border: 'border-slate-800', head: 'bg-slate-800 text-slate-200' },
        INTER_VIP: { bg: 'bg-slate-950 text-blue-50', border: 'border-blue-700', head: 'bg-blue-900 text-yellow-400' }
    };
    ...
    function renderThemeToggle() {
        ...
        btn.className = 'px-3 py-2 rounded-full text-[10px] font-bold transition-colors shadow-lg flex items-center gap-1.5 border ' +
            (light
        ...
        btn.innerHTML = (light ? '<i class="fas fa-sun"></i> <span>Light</span>' : '<i class="fas fa-moon"></i> <span>Dark</span>');
        ...
        if (lab) lab.className = 'text-[10px] font-bold mr-1 uppercase tracking-widest ' + (light ? 'text-stone-600' : 'text-slate-400');
    }
    ...
    function buatPartikelSakura() {
        ...
        for (var i = 0; i < N; i++) {
            ...
            var size = isFar ? 5 + Math.random() * 4      // 5-9px
                    : isHero ? 16 + Math.random() * 8     // 16-24px
    ...
    function setSakuraParticles(visible) {
        var box = document.getElementById('sakura-particles');
        if (!box) return;
        if (visible) { buatPartikelSakura(); box.classList.remove('hidden'); }
    ...
    // ==========================================
    function safeSetVal(id, value) { var el = document.getElementById(id); if (el) el.value = value || ''; }
    ...
    function showToast(message, type = 'success') {
        ...
        const bgColor = type === 'success' ? 'bg-emerald-600 border-emerald-400' : (type === 'error' ? 'bg-red-600 border-red-400' : 'bg-sky-600 border-sky-400');
        const icon = type === 'success' ? 'fa-check-circle' : (type === 'error' ? 'fa-exclamation-triangle' : 'fa-info-circle');
        toast.className = 'flex items-center gap-3 px-5 py-3.5 rounded-xl border text-white text-sm font-bold shadow-2xl transform transition-transform duration-300 translate-x-full ' + bgColor;
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('fixed') && e.target.classList.contains('inset-0')) { e.target.classList.add('hidden'); }
    });

    function safeSet(id, value) { var el = document.getElementById(id); if (el) { el.innerHTML = value; } }
    ...
    function formatPendidikanTingkat(p) {
        ...
        if (typeof p === 'string') {
            var t = p.trim();
            if (t.startsWith('[')) { try { arr = JSON.parse(t); } catch (e) { arr = null; } }
    ...
    function muatVendorLib(nama) {
        ...
        if (!_vendorPromises[nama]) {
            _vendorPromises[nama] = new Promise(function (resolve, reject) {
                ...
                s.src = src;
                s.onload = resolve;
                s.onerror = function () { delete _vendorPromises[nama]; reject(new Error('vendor gagal dimuat: ' + nama)); };
    ...
    async function renderExcelKeFrame(frame, url) {
        if (typeof XLSX === 'undefined') {
            try { await muatVendorLib('xlsx'); } catch (e) { return false; }
        }
        ...
        try {
            ...
            var doc = '<!doctype html><html><head><meta charset="utf-8"><title>' + nama + '</title>' +
                ...
                '</head><body><div style="margin-bottom:10px;color:#94a3b8;font-size:12px">📊 ' + nama + '</div>' + html + '</body></html>';
    ...
    function _pasangTimerPreviewFallback(frame, url) {
        ...
        frame.addEventListener('load', function _once() {
            clearTimeout(t);
    ...
    function pesanLoadingPreview() {
        ...
        return '<div style="font-family:system-ui,sans-serif;background:#0f172a;color:#e2e8f0;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;text-align:center;padding:28px;box-sizing:border-box">' +
    ...
    function pesanPreviewTidakTersedia(url) {
        ...
        return '<div style="font-family:system-ui,sans-serif;background:#0f172a;color:#e2e8f0;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;text-align:center;padding:28px;box-sizing:border-box">' +
            '<div style="font-size:22px">📄</div>' +
            '<div style="font-size:16px;font-weight:700">' + judul + (ext ? ' <span style="opacity:.6">(.' + ext + ')</span>' : '') + '</div>' +
            '<div style="font-size:13px;opacity:.75;max-width:320px">' + hint + '</div>' +
            '<a href="' + String(url).replace(/"/g, '&quot;') + '" target="_blank" rel="noopener" style="padding:10px 26px;background:#0284c7;color:#fff;border-radius:999px;text-decoration:none;font-weight:700"><i class="fas fa-download"></i> ' + unduh + '</a>' +
            '</div>';
    }
    ...
    function populate(id, list) { 
        ...
        if(list && list.length > 0) { for(var i=0; i<list.length; i++) html += '<option value="' + trOptionId(list[i]) + '">' + trOption(list[i]) + '</option>'; }
        el.innerHTML = html; 
    }
    ...
    function populateCheckboxes(id, list, nameAttr) { 
        ...
        if(list && list.length > 0) { for(var i=0; i<list.length; i++) { html += '<label class="flex items-center gap-2 cursor-pointer p-1 hover:bg-white/10 rounded text-slate-300"><input type="checkbox" name="' + nameAttr + '" value="' + trOptionId(list[i]) + '" class="accent-red-500"> ' + trOption(list[i]) + '</label>'; } }
        el.innerHTML = html; 
    }
    
    function formatInputWA(el) { 
        let val = el.value.replace(/\D/g, ''); 
        if (val.startsWith('0')) { val = '62' + val.substring(1); } else if (val.startsWith('8')) { val = '62' + val; } 
        el.value = val.length > 0 ? '+' + val : ''; 
    }
    
    function salinTeksDecode(encodedText) { 
        var el = document.createElement('textarea'); el.value = decodeURIComponent(encodedText); 
        document.body.appendChild(el); el.select(); 
        try { document.execCommand('copy'); showToast(tr('alert.success'), 'success'); } catch (err) {} 
        document.body.removeChild(el); 
    }
    
    function toggleMinimize(id, btnEl) {
        const el = document.getElementById(id); const icon = btnEl.querySelector('i.fa-chevron-down');
        if(el) el.classList.toggle('hidden'); if(icon) icon.classList.toggle('rotate-180');
    }

    function applyTheme(theme) { 
        CURRENT_THEME = theme; var cfg = THEMES[theme]; if(!cfg) return; 
        ...
        if (bar) {
            bar.className = 'flex flex-wrap justify-between items-center p-4 rounded-xl border shadow-lg mb-6 gap-4 transition-colors ' +
            ...
            bar.querySelectorAll('span.text-slate-400').forEach(function (s) {
                s.className = s.className.replace('text-slate-400', light ? 'text-stone-600' : 'text-slate-400');
            });
        }
        ...
        if (overlay) overlay.className = 'absolute inset-0 rounded-[2.5rem] transition-colors duration-700 ' +
            (light ? 'bg-gradient-to-t from-black/90 via-black/60 to-black/60' : 'bg-gradient-to-t from-black/95 via-black/40 to-transparent');
        ...
        if (tagline) tagline.className = 'text-xs md:text-sm font-bold tracking-[4px] mb-1 ' + (light ? 'text-white' : 'text-pink-300');
        var fOverlay = document.getElementById('asj-footer-overlay');
        if (fOverlay) fOverlay.className = 'absolute inset-0 backdrop-blur-sm transition-colors duration-700 ' + (light ? 'bg-black/65' : 'bg-black/80');
        ...
        try { localStorage.setItem('asj_theme', theme); } catch(e) {}
        renderThemeToggle();
     ...
     function changePage(page) { 
         var pPub = document.getElementById('page-public'); if(pPub) pPub.classList.toggle('hidden', page !== 'public'); 
         var pAdm = document.getElementById('page-admin'); if(pAdm) pAdm.classList.toggle('hidden', page !== 'admin'); 
         var pKan = document.getElementById('page-kandidat'); if(pKan) pKan.classList.toggle('hidden', page !== 'kandidat'); 
     ...
     function logoutApp() { 
         ...
         var nMod = document.getElementById('nav-mode'); if(nMod) nMod.classList.remove('hidden'); 
         ...
         // sudah tidak login.
         var mLg = document.getElementById('mobile-nav-logged-out'); if(mLg) mLg.classList.remove('hidden');
         var mAd = document.getElementById('mobile-nav-admin'); if(mAd) mAd.classList.add('hidden');
         var mKa = document.getElementById('mobile-nav-kandidat'); if(mKa) mKa.classList.add('hidden');
         isAdmin = false; isKandidat = false; currentAdminName = ''; currentKandidatName = ''; currentKandidatWa = ''; currentKandidatId = ''; 
