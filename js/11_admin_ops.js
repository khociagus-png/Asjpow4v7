    ...
    function getStatusWaktu(waktuStr) {
        if(!waktuStr || waktuStr === '-') return {text: '-', color: 'text-slate-400', bg: 'bg-slate-800'};
        let t = new Date(waktuStr.replace(' ', 'T'));
        if(isNaN(t)) return {text: waktuStr, color: 'text-slate-400', bg: 'bg-slate-800'};
        
        let now = new Date(); let diff = t - now;
        let diffDays = Math.ceil(diff / (1000 * 60 * 60 * 24));
        let diffHours = diff / (1000 * 60 * 60);
        
        if (diff < 0 && diffHours > -24) return {text: 'ONGOING', color: 'text-white', bg: 'bg-emerald-600 animate-pulse'};
        if (diff < 0) return {text: 'SELESAI', color: 'text-slate-400', bg: 'bg-slate-800'};
        if (diffHours <= 1.5) return {text: 'SEGERA', color: 'text-white', bg: 'bg-red-600 animate-pulse'};
        if (diffDays === 0 || diffDays === 1) return {text: 'HARI INI', color: 'text-amber-900', bg: 'bg-amber-400'};
    ...
    function renderDashboardAgenda() {
        var list = document.getElementById('dash-agenda-list'); if (!list) return;
        var upcoming = ALL_SCHEDULES.filter(s => {
            let diff = new Date(s.waktu.replace(' ', 'T')) - new Date(); return !isNaN(diff) && diff > -86400000;
        }).sort((a,b) => new Date(a.waktu.replace(' ', 'T')) - new Date(b.waktu.replace(' ', 'T'))).slice(0, 5);
        ...
        if (upcoming.length === 0) html = '<div class="text-center text-slate-500 py-6 text-xs font-bold border border-dashed border-slate-700 rounded-xl bg-black/20">' + tr('ui.agenda_empty') + '</div>';
        list.innerHTML = html;
    }

    function renderJadwal() {
        var tb = document.getElementById('admin-jadwal-body'); if (!tb) return; var html = '';
        for (var i = 0; i < Math.min(ALL_SCHEDULES.length, limitJad); i++) {
            var j = ALL_SCHEDULES[i]; var s = getStatusWaktu(j.waktu);
        ...
        if (ALL_SCHEDULES.length > limitJad) { html += '<tr><td colspan="5" class="p-4 text-center"><button onclick="limitJad+=10; renderJadwal();" class="text-xs text-amber-400 font-bold">' + tr('form.txt_lebih_banyak') + '</button></td></tr>'; }
        tb.innerHTML = html;
    }
    ...
    async function bukaModalListKandidat(code) {
        ...
        var job = ALL_DB_JOBS.find(j => j.code === code); var cands = ALL_CANDIDATES.filter(c => c.idLoker && c.idLoker.includes(code)); if (!job) return;
        safeSet('list-job-code', code);
        var html = ''; var txt = "*LIST KANDIDAT JOB " + code + "* Total: " + cands.length + " Pelamar \n\n";
        
        if (cands.length === 0) { html = '<div class="text-center text-slate-500 py-4">' + tr('ui.no_applicants') + '</div>'; txt += tr('ui.no_candidates_empty'); } 
        else {
            cands.forEach((c, i) => {
        ...
        var lc = document.getElementById('list-kandidat-content'); if(lc) lc.innerHTML = html;
    ...
    async function mulaiKirimUndanganGrup() {
        ...
        if(!linkGrup) { showToast(tr('ui.toast_group_link_required'), "error"); return; }
        
        if (typeof window.ensureAllCandidates === 'function') { try { await window.ensureAllCandidates(); } catch (e) {} }
        let cands = ALL_CANDIDATES.filter(c => c.idLoker && c.idLoker.includes(jobCode));
        if(cands.length === 0) { showToast(tr('ui.toast_no_cand_in_job'), "error"); return; }
        
        let btn = document.getElementById('btn-undang-grup');
        btn.innerHTML = tr('ui.sending'); btn.disabled = true;
        ...
        btn.innerHTML = tr('ui.start_send_invite'); btn.disabled = false;
    }
        ...
        try {
            ...
            if(res.success) {
                ...
                if(res.data.length === 0) {
                    html = '<tr><td colspan="4" class="p-8 text-center text-slate-500 font-bold italic">' + tr('ui.no_students') + '</td></tr>';
                } else {
                    res.data.forEach((s, i) => {
                        let gBadge = (s.jenis_kelamin || s.gender) === 'L' 
    ...
    const CONFIG_CATEGORIES = [
        { key: 'tahapan', label: 'Tahapan Seleksi (Pipeline)', color: 'purple', hint: 'Urutan: CHECK KAIWA → MENDAN → MENSETSU → LOLOS USER → MCU PARPOR → TTD KONTRAK → PROSES COE → VISA → FLIGHT', order: 1 },
        { key: 'statusLamaran', label: 'Status Lamaran', color: 'emerald', hint: 'MENUNGGU, REVIEW ADMIN, GAGAL, LULUS', order: 2 },
        { key: 'statusLoker', label: 'Status Loker Publik', color: 'red', hint: 'OPEN, URGENT, CLOSE', order: 3 },
        ...
        CONFIG_CATEGORIES.forEach(cat => {
            ...
            if (items.length === 0) {
                ...
                items.forEach((item, index) => {
                    let num = (cat.key === 'tahapan') ? '<span class="mr-1 text-slate-500 font-black">' + (index + 1) + '.</span>' : '';
    ...
    function tambahConfigItem(key) {
        ...
        let ada = DROPDOWNS[key].some(function(ex) { return trOptionId(ex) === valId; });
        if (ada) {
    ...
    function setSkeletonLoading(elementId, cols) {
        ...
        for (var i = 0; i < 5; i++) {
            ...
            for (var j = 0; j < cols; j++) {
                ...
                html += '<td class="p-4"><div class="h-4 bg-slate-700/60 rounded animate-pulse ' + w + '"></div></td>';
            }
            html += '</tr>';
        }
        tb.innerHTML = html;
    ...
    function jalankanSemuaSkeleton() {
        ...
        if(isAdmin) {
            ...
            if(document.getElementById('admin-kandidat-body')) setSkeletonLoading('admin-kandidat-body', 6);
        ...
        if(isKandidat && kRiwayat) {
            kRiwayat.innerHTML = '<div class="p-3.5 rounded-xl border border-slate-700/50 bg-black/40 mb-2 animate-pulse"><div class="h-4 bg-slate-700/60 rounded w-1/2 mb-2"></div><div class="h-3 bg-slate-700/60 rounded w-1/3"></div></div>' + 
    ...
    function renderMigrasiResults(list) {
        ...
        box.innerHTML = list.map(function (r) {
            var color = r.status === 'fail' ? 'text-rose-300' : r.status === 'warn' ? 'text-amber-300' : 'text-slate-300';
            return '<div class="flex items-start gap-2 text-[11px] ' + color + '">' +
                '<span class="mt-0.5">' + (icons[r.status] || icons.skip) + '</span>' +
                '<span><b>' + r.step + ':</b> ' + r.detail + '</span></div>';
        }).join('');
    ...
    function renderMigrasiDriveList() {
        ...
        if (!DRIVE_CANDIDATES.length) {
            box.innerHTML = '<div class="bg-emerald-900/20 border border-emerald-500/40 rounded-xl p-6 text-center"><p class="text-emerald-400 font-black text-sm mb-1"><i class="fas fa-check-circle mr-1"></i> ' + tr('ui.all_on_storage') + '</p><p class="text-[10px] text-slate-400">' + tr('ui.no_drive_links') + '</p></div>';
            return;
        }
        var html = '';
        DRIVE_CANDIDATES.forEach(function (c, i) {
            var chips = c.fields.map(function (f) {
                var info = { PAS_PHOTO: ['PAS PHOTO', 'image'], CV: ['CV', 'file-alt'], JFT: ['JFT', 'file-pdf'], SSW: ['SSW', 'file-pdf'] }[f] || [f, 'file'];
                return '<span class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-900/40 text-amber-300 border border-amber-500/40 rounded-full text-[9px] font-bold"><i class="fas fa-' + info[1] + ' text-[8px]"></i> ' + info[0] + '</span>';
            }).join('');
            html += '<div class="bg-black/40 border border-slate-700 rounded-xl p-3">' +
                '<div class="flex items-center justify-between gap-2 flex-wrap mb-2">' +
                    '<div class="min-w-0"><p class="text-xs font-black text-white truncate">' + c.nama + '</p>' +
                    '<p class="text-[9px] text-slate-500 font-mono">' + c.idKandidat + ' · ' + c.wa + '</p></div>' +
                    '<div class="flex flex-wrap gap-1">' + chips + '</div>' +
                '</div>' +
                '<div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">' +
                c.fields.map(function (f) { return migrasiDriveFieldHtml(c, f); }).join('') +
                '</div>' +
            '</div>';
        });
        box.innerHTML = html;
    ...
    function migrasiDriveFieldHtml(c, field) {
        var info = { PAS_PHOTO: ['PAS PHOTO', 'PAS_PHOTO'], CV: ['CV', 'CV'], JFT: ['JFT', 'JFT'], SSW: ['SSW', 'SSW'] }[field] || [field, field];
        var safeId = (c.idKandidat + '_' + field).replace(/[^A-Z0-9_]/gi, '');
        return '<div class="bg-slate-900/60 border border-slate-700 rounded-lg p-2.5">' +
            '<div class="flex items-center justify-between mb-1.5">' +
                '<label class="text-[9px] font-bold text-slate-300 uppercase"><i class="fas fa-link text-rose-400 mr-1"></i> ' + info[0] + ' (Drive)</label>' +
                '<span id="dl-st-' + safeId + '" class="text-[9px]"></span>' +
            '</div>' +
            '<div class="flex gap-1.5">' +
                '<input type="file" id="dl-file-' + safeId + '" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" class="flex-1 text-[9px] text-slate-300 file:mr-2 file:px-2 file:py-1 file:rounded-md file:border-0 file:bg-slate-700 file:text-white file:text-[9px] file:font-bold">' +
                '<button type="button" onclick="uploadDriveField(\'' + c.idKandidat + '\', \'' + c.nama.replace(/'/g, "\\'") + '\', \'' + field + '\')" class="px-3 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-[9px] font-black transition flex-shrink-0"><i class="fas fa-upload mr-1"></i> Upload</button>' +
            '</div>' +
        '</div>';
    }
    ...
    function driveSetStatus(id, label, state, msg) {
        ...
        if (!el) return;
        if (state === 'uploading') el.innerHTML = '<span class="text-sky-300"><i class="fas fa-spinner fa-spin mr-0.5"></i> ' + tr('ui.file_uploading') + '</span>';
        ...
        else if (state === 'fail') el.innerHTML = '<span class="text-rose-400"><i class="fas fa-times-circle mr-0.5"></i> ' + tr('ui.file_failed') + '</span>';
    }
    ...
    async function uploadDriveField(idKandidat, nama, field) {
        ...
        if (extErr) { showToast(extErr, 'error'); return; }
        ...
        try {
            ...
            if (res && res.success) {
                ...
                var c = DRIVE_CANDIDATES.find(function (x) { return x.idKandidat === idKandidat; });
                if (c) {
                    c.fields = (c.fields || []).filter(function (f) { return f !== field; });
                    if (!c.fields.length) DRIVE_CANDIDATES = DRIVE_CANDIDATES.filter(function (x) { return x.idKandidat !== idKandidat; });
                }
                setTimeout(function () { renderMigrasiDriveList(); muatMigrasiDrive(); }, 1200);
            } else {
