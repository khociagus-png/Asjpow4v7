...
function validateStep1() {
  if (!$('job').value) {
    alert('Nomor Job tidak terdeteksi!');
    return false;
  }
  if (!$('nama').value.trim()) {
  ...
  if (!$('tb').value) {
    alert('Tinggi Badan wajib diisi.');
    $('tb').focus();
    return false;
  }
  if (!$('bb').value) {
...
function validateStep2() {
  ...
  if ($('photo').files.length === 0 && oldPhotoUrl === '-') {
    alert('Silakan Upload Pas Photo Anda.');
    return false;
  }
  if (!$('card-cv').classList.contains('hidden') && $('cv').files.length === 0) {
    alert('Silakan Upload Dokumen CV / Format TSK Anda.');
    return false;
  }
  if (
    ...
    alert('Silakan Upload Sertifikat JFT Anda.');
    return false;
  }
  if (
    ...
    alert('Silakan Upload Sertifikat SSW Anda.');
    return false;
  }
