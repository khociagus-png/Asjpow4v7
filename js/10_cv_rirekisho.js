    ...
    function renderCVAjaib(d, fotoUrl, waTarget) {
        ...
        let ai = {};
        try { if (d.AIDATAJSON && d.AIDATAJSON !== '-') ai = JSON.parse(d.AIDATAJSON); } catch(e){}
        ...
        const getArr = (key) => {
            ...
            if (aiArr) { try { let p = JSON.parse(aiArr); if (Array.isArray(p) && p.length) return p; } catch(e) { if (Array.isArray(aiArr)) return aiArr; } }
            return [];
        };
        ...
        if (tglAsli !== '-') {
           ...
           if (!isNaN(dt)) {
               tglFormat = dt.getFullYear() + "年" + String(dt.getMonth()+1).padStart(2,"0") + "月" + String(dt.getDate()).padStart(2,"0") + "日";
           } else {
               tglFormat = tglAsli;
           }
        }
        ...
        let fotoHtml = fotoUrl ? `<img src="${fotoUrl}" style="width: 100%; height: 100%; min-height: 195px; object-fit: cover; object-position: top center; display: block; margin: 0; padding: 0;">` : `<div style="width: 100%; min-height: 195px; display: flex; align-items: center; justify-content: center; font-size: 10px; color: gray;">FOTO</div>`;
        ...
        let html = buildCvKertasA4({ v, fotoHtml, btnPrintHtml, tglFormat, waTarget,
            ...idn, eduHtml, jobHtml, famHtml
        });
    ...
    function cetakCVRirekisho() {
        if (!isAdmin) { showToast(tr('ui.toast_rireki_admin_only'), 'error'); return; }
        window.print();
    }
