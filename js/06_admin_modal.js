    ...
    // ==========================================
    function setFilterBidang(v) { dbFilterBidang = v; renderDbFilters(); filterDbJob(); }
    function setFilterTahapan(v) { dbFilterTahapan = v; renderDbFilters(); filterDbJob(); }
    function setSortDb(t) {
        dbSortType = t;
        ['terbaru', 'terlama', 'terbanyak'].forEach(x => {
            var b = document.getElementById('btn-sort-' + x);
            if (b) b.className = 'px-4 py-1.5 rounded-full font-bold transition ' + (t === x.toUpperCase() ? 'bg-purple-600 text-white shadow-lg' : 'bg-slate-700 text-slate-300 hover:bg-slate-600');
        });
        filterDbJob();
    }

    function renderDbFilters() {
        var bContainer = document.getElementById('filter-bidang-container'); var tContainer = document.getElementById('filter-tahapan-container');
        if (DROPDOWNS.kategori && bContainer) {
            var bHtml = '<button onclick="setFilterBidang(\'ALL\')" class="px-3 py-1 rounded-full ' + (dbFilterBidang === 'ALL' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400') + '">' + tr('public.all') + '</button>';
            // Label chip dwi bahasa (trOption); onclick tetap ID asli (trOptionId)
            // supaya filter cocok dengan data yang tersimpan.
            DROPDOWNS.kategori.forEach(kat => { bHtml += '<button onclick="setFilterBidang(\'' + trOptionId(kat) + '\')" class="px-3 py-1 rounded-full ' + (dbFilterBidang === trOptionId(kat) ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400') + '">' + trOption(kat) + '</button>'; });
            bContainer.innerHTML = bHtml;
        }
        if (DROPDOWNS.tahapan && tContainer) {
            var tHtml = '<button onclick="setFilterTahapan(\'ALL\')" class="px-3 py-1 rounded-full ' + (dbFilterTahapan === 'ALL' ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400') + '">' + tr('public.all') + '</button>';
            DROPDOWNS.tahapan.forEach(thp => { tHtml += '<button onclick="setFilterTahapan(\'' + trOptionId(thp) + '\')" class="px-3 py-1 rounded-full ' + (dbFilterTahapan === trOptionId(thp) ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-400') + '">' + trOption(thp) + '</button>'; });
    ...
    async function bukaDigitalCV(id) {
        ...
        try {
            ...
            if (!c) { showToast(tr('ui.toast_profile_not_found2'), 'error'); return; }
            ...
            let miniFilled = 0; miniFields.forEach(f => { if(f && String(f).trim() !== '' && String(f).trim() !== '-') miniFilled++; });
            ...
            let masterFilled = 0; masterFields.forEach(f => { if(f && String(f).trim() !== '' && String(f).trim() !== '-') masterFilled++; });
            ...
            cvBadges += '<i class="fas fa-medal text-orange-500 text-lg drop-shadow-[0_0_8px_rgba(249,115,22,0.8)]" title="' + tr('ui.badge_bronze') + '"></i>';
            if(pMini === 100) cvBadges += '<i class="fas fa-award text-slate-300 text-lg drop-shadow-[0_0_8px_rgba(203,213,225,0.8)]" title="' + tr('ui.badge_silver') + '"></i>';
            if(pMaster === 100) cvBadges += '<i class="fas fa-crown text-yellow-400 text-xl drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]" title="' + tr('ui.badge_gold') + '"></i>';
            ...
            if (c.tglLahir && c.tglLahir !== '-' && c.tglLahir.trim() !== '') {
                ...
                if (!isNaN(dob)) {
                    let today = new Date(); let age = today.getFullYear() - dob.getFullYear();
            ...
            if (passRow && passEl) {
                if (isAdmin) {
                    if (c.passwordDiubah) {
                        ...
                        var pass4 = String(c.wa || '').replace(/\D/g, '').slice(-4);
                        if (pass4) {
            ...
            var img = document.getElementById('cv-foto'); var icn = document.getElementById('cv-no-foto');
            if (img && icn) {
                img.classList.add('hidden'); icn.classList.remove('hidden');
                if (c.pasPhoto && c.pasPhoto !== '-' && c.pasPhoto.length > 5) {
                    var finalUrl = getHighResImage(c.pasPhoto);
                    img.onload = function() { img.classList.remove('hidden'); icn.classList.add('hidden'); };
            ...
            if (isAdmin) {
                if (c.jftUrl && c.jftUrl !== '-' && c.jftUrl.toLowerCase().startsWith('http')) { btnJft.onclick = function() { bukaInlinePreview(c.jftUrl); }; btnJft.classList.remove('hidden'); } else { if (btnJft) btnJft.classList.add('hidden'); }
                if (c.sswUrl && c.sswUrl !== '-' && c.sswUrl.toLowerCase().startsWith('http')) { btnSsw.onclick = function() { bukaInlinePreview(c.sswUrl); }; btnSsw.classList.remove('hidden'); } else { if (btnSsw) btnSsw.classList.add('hidden'); }
                if (c.cvUrl && c.cvUrl !== '-' && c.cvUrl.toLowerCase().startsWith('http')) { btnCv.onclick = function() { bukaInlinePreview(c.cvUrl); }; btnCv.classList.remove('hidden'); } else { if (btnCv) btnCv.classList.add('hidden'); }
            } else {
            ...
            if (notesArea) {
                if (isAdmin) {
                    ...
                    document.getElementById('cv-catatan-int').value = catatanIntStr.replace(/\[VIP\]/gi, '').replace(/\[(?:KELAS\s*[A-Z0-9]+|[A-Z0-9]+)\]/gi, '').trim();
            ...
            let isLolos = /LOLOS|PEMBERKASAN|MCU|MEDICAL|MEDIKAL|PARPOR|PASPOR|PASPORT|MATCH|TERIMA|SIAP|TTD|KONTRAK|VISA|COE|KTKLN|SISKOP|FLIGHT|BERANGKAT|TERBANG|TIKET|E-ID/i.test(c.tahapan || '');
            ...
            let btnFolder = document.getElementById('btn-cv-folder');
                ...
                if (isAdmin && isLolos) {
                    ...
                    if (bCv) { if (c.cvUrl && c.cvUrl !== '-') { bCv.onclick = function() { bukaPreviewDokumen(c.cvUrl); }; bCv.classList.remove('hidden'); } else bCv.classList.add('hidden'); }
                    
                    var bJft = document.getElementById('btn-cv-file-jft');
                    if (bJft) { if (c.jftUrl && c.jftUrl !== '-') { bJft.onclick = function() { bukaPreviewDokumen(c.jftUrl); }; bJft.classList.remove('hidden'); } else bJft.classList.add('hidden'); }
                    
                    var bSsw = document.getElementById('btn-cv-file-ssw');
                    if (bSsw) { if (c.sswUrl && c.sswUrl !== '-') { bSsw.onclick = function() { bukaPreviewDokumen(c.sswUrl); }; bSsw.classList.remove('hidden'); } else bSsw.classList.add('hidden'); }
                    
                    var bPhoto = document.getElementById('btn-cv-file-photo');
                    if (bPhoto) { if (c.pasPhoto && c.pasPhoto !== '-') { bPhoto.onclick = function() { bukaPreviewDokumen(c.pasPhoto); }; bPhoto.classList.remove('hidden'); } else bPhoto.classList.add('hidden'); }
        ...
        } catch (err) { showToast(tr('ui.toast_load_profile_failed'), 'error'); }
    }
    ...
    function isiEditCepatCv(c) {
        ...
        if (!btn || !form) return;
        if (!isAdmin) { btn.classList.add('hidden'); form.classList.add('hidden'); return; }
        ...
        if (usia && !usia.dataset.touched) usia.value = (c.usia && c.usia !== '-' && c.usia !== '') ? c.usia : '';
        var tmp = document.getElementById('cv-edit-tempat-lahir');
        if (tmp && !tmp.dataset.touched) tmp.value = (c.tempatLahir && c.tempatLahir !== '-') ? c.tempatLahir : '';
        ...
        if (tb && !tb.dataset.touched) tb.value = (c.tb && c.tb !== '-' && c.tb !== '') ? String(c.tb).replace(/\D/g, '') : '';
        var bb = document.getElementById('cv-edit-bb');
        if (bb && !bb.dataset.touched) bb.value = (c.bb && c.bb !== '-' && c.bb !== '') ? String(c.bb).replace(/\D/g, '') : '';
    ...
    async function simpanEditCepatCv() {
        ...
        if (!wa) { showToast(tr('ui.toast_data_not_found'), 'error'); return; }
        var payload = {
            wa: wa,
        ...
        try {
            ...
            if (res && res.success) {
                ...
                ['cv-edit-gender','cv-edit-usia','cv-edit-tempat-lahir','cv-edit-tgl-lahir','cv-edit-tb','cv-edit-bb','cv-edit-jft','cv-edit-ssw'].forEach(function(id) {
    ...
    function copyInfoLoker(c) {
        var j = ALL_JOBS.find(x => x.code === c); if (!j) return;
        var txt = "*INFO LOKER ASJ*  Posisi: " + j.pekerjaan + "  Lokasi: " + j.lokasi + "  Gender: " + j.gender + "  *Syarat:*  " + j.syarat + "  *Ket:*  " + j.keterangan + "  Daftar via portal resmi.";
