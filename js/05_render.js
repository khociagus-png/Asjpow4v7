...
function renderAdmin(filteredJobs) {
  ...
  if (sourceArray.length > limitAdm) {
    html +=
      '<tr><td colspan="5" class="p-4 text-center"><button onclick="limitAdm+=10; renderAdmin();" class="text-xs text-red-400">' +
      tr('button.more') +
      '</button></td></tr>';
  }
  tb.innerHTML = html;
...
function renderDbJobTable(arr) {
  ...
  if (arr.length > limitDb) {
    html +=
      '<tr><td colspan="6" class="p-4 text-center"><button onclick="limitDb+=10; filterDbJob();" class="text-xs text-purple-400 font-bold">' +
