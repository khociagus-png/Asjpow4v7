...
function mapJob(row) {
  ...
  const templateCv =
    pick(row, ["format_cv", "template_cv", "templatecv", "cv_template"]) || "";
  return {
    ...
    keterangan: toText(
      pick(row, ["keterangan", "deskripsi", "description", "info_tambahan"])
    ),
    tahapan: toText(pick(row, ["tahapan", "tahapan_seleksi"])),
    tsk: toText(pick(row, ["tsk", "pengurus"])),
    dokumenShare: toText(
      pick(row, ["dokumen_share", "dokumenshare", "dokumen", "docs_share"])
    ),
    template: templateCv,
    templateCv,
    pamflet: toText(pick(row, ["link_pamflet", "pamflet", "poster", "flyer", "brosur"])),
    rincianBiaya: toText(
      pick(row, ["rincian_biaya", "rincianbiaya", "biaya_rincian", "rincian"])
    ),
    totalBiaya: toText(pick(row, ["total_biaya", "totalbiaya", "total"])),
    createdAt: toText(
      pick(row, ["created_at", "createdat", "tanggal", "date_created", "tgl"])
    ),
    _raw: row,
  };
}
  ...
  return {
    ...
    tbBb:
      (tb && tb !== "-") || (bb && bb !== "-")
        ...
        : "-",
    ttl:
      [tempatLahir, tglLahir].filter((x) => x && x !== "-").join(", ") || "-",
...
async function queryPaged(table, { page = 1, pageSize = 50, q = "" } = {}) {
  ...
  const res = await fetch(
    supabaseUrl().replace(/\/$/, "") + "/rest/v1/" + table + "?" + qs,
    {
      method: "GET",
      headers: {
        apikey: supabaseKey(),
...
async function findAssets() {
  ...
  for (const row of found.rows) {
    ...
    if (logo) {
      const nested = (k) =>
        (row.assets && typeof row.assets === "object" && row.assets[k]) || null;
      return {
        LOGO: logo,
        BANNER: {
          TOKYO:
            ...
            null,
          SAKURA:
            pick(row, ["banner_sakura", "banner_sakura_url"]) ||
            nested("BANNER")?.SAKURA ||
            null,
        },
        FOOTER: {
          TOKYO:
            ...
            null,
          SAKURA:
            pick(row, ["footer_sakura"]) || nested("FOOTER")?.SAKURA || null,
        },
        SOCIAL: {
          whatsapp:
            ...
            null,
          instagram:
            pick(row, ["ig", "instagram", "social_ig"]) ||
            nested("SOCIAL")?.instagram ||
            null,
          tiktok:
            pick(row, ["tiktok", "social_tiktok"]) ||
            nested("SOCIAL")?.tiktok ||
            null,
          maps:
            pick(row, ["maps", "maps_link", "lokasi_maps"]) ||
            nested("SOCIAL")?.maps ||
            null,
        },
      };
...
const BIO_COLUMNS = [
  ["email", ["email"]], ["tmplahir", ["tempat_lahir", "tmplahir"]], ["tgllahir", ["tgl_lahir", "tgllahir"]],
  ["alamat", ["alamat_lengkap", "alamat"]], ["ayah", ["nama_ayah", "ayah"]], ["ttlayah", ["ttl_ayah", "ttlayah"]],
  ["ibu", ["nama_ibu", "ibu"]], ["ttlibu", ["ttl_ibu", "ttlibu"]], ["pasport", ["no_pasport", "pasport"]],
  ["coe", ["no_coe", "coe"]], ["kotapasport", ["kota_pasport", "kotapasport"]], ["tglpasport", ["tgl_pasport", "tglpasport"]],
  ["exppasport", ["exp_pasport", "exppasport"]], ["pt", ["nama_perusahaan", "pt"]], ["shacou", ["nama_shacou", "shacou"]],
  ["telppt", ["telp_perusahaan", "telppt"]], ["webpt", ["web_perusahaan", "webpt"]], ["alamatpt", ["alamat_perusahaan", "alamatpt"]],
];
