...
function publicUrl(path) {
  return supabase.supabaseUrl().replace(/\/$/, "") + "/storage/v1/object/public/" + bucket() + "/" + path;
}
...
async function handleGetUploadUrls(payload, sessionToken) {
  ...
  try {
    for (const f of files) {
      ...
      urls[key] = {
        signedUrl: supabase.supabaseUrl().replace(/\/$/, "") + "/storage/v1" + (rel.startsWith("/") ? rel : "/" + rel),
...
async function handleIsJobRequiresCv(payload) {
  ...
  try {
    ...
    const job = found.rows.find((r) => String(supabase.pick(r, ["code_job", "code"]) || "") === code);
...
async function handleSubmitApply(payload) {
  ...
  try {
    ...
    const missing = share.filter((req) => !have.has(req) && !["CV", "JFT", "SSW"].includes(req) && req !== "-");
    const missingCore = share.filter((req) => ["CV", "JFT", "SSW"].includes(req) && !have.has(req));
    if (missingCore.length || missing.length) {
      return {
        success: false,
        message:
          "Berkas belum lengkap. Harap upload: " +
          [...missingCore, ...missing].join(", "),
      };
    }
...
async function handleGetExistingCandidateJsonByWa(payload) {
  ...
  try {
    ...
    const want = supabase.normalizeWa(wa);
    const row = found.rows.find((r) => supabase.normalizeWa(String(supabase.pick(r, APPLY_WA_COLS) || "")) === want);
...
const MASTER_COLUMN_MAP = {
  ...
  mataKiri: "mata_kiri", mataKanan: "mata_kanan", kacamata: "kacamata", butaWarna: "buta_warna",
  tato: "tato", tindik: "tindik", merokok: "merokok", alkohol: "minum_alkohol",
  ...
  keahlianKhusus: "keahlian_khusus", hobi: "hobi_dan_keterampilan",
  ...
  lamaJepang: "status_eks_jepang", gajiYen: "harapan_gaji_yen", tabungan: "harapan_tabungan",
  bhsJepang: "bahasa", nilai: "jft", lisensi: "bidangssw", ssw: "ssw",
  noPaspor: "no_paspor", tglTerbitPaspor: "tgl_terbit_pasport", expPaspor: "exp_pasport",
...
async function handleGetMasterDataByWa(payload, sessionToken) {
  ...
  try {
    ...
    const out = {
      ...
      PANGGILAN_KATAKANA: v("panggilan_katakana"), TEMPAT_LAHIR: v("tempat_lahir"),
      TGL_LAHIR: v("tgl_lahir"), GENDER: v("gender"), USIA: v("usia"), AGAMA: v("agama"),
      STATUS_PERNIKAHAN: v("status_pernikahan"), JUMLAH_ANAK: v("jumlah_anak"),
      NIK: v("nik"), DRIVER_LICENSE: v("driver_license"), ALAMAT_LENGKAP: v("alamat_lengkap"),
      EMAIL: v("email"), TT: v("tb"), TB: v("tb"), BB: v("bb"), GOLONGAN_DARAH: v("golongan_darah"),
      ...
      HARAPAN_TABUNGAN: v("harapan_tabungan"), BAHASA: v("bahasa"), JFT: v("jft"),
      SSW: v("ssw"), BIDANGSSW: v("bidangssw"), PROMOSI_DIRI: v("promosi_diri"),
      KELEBIHAN: v("kelebihan"), KEKURANGAN: v("kekurangan"), KEAHLIAN_KHUSUS: v("keahlian_khusus"),
      "HOBI_&_KETERAMPILAN": v("hobi_dan_keterampilan"), ALASAN_MEMILIH_BIDANG: v("alasan_memilih_bidang"),
      MOTIVASI_KE_JEPANG: v("motivasi_ke_jepang"), KEINGINAN_PRIBADI: v("keinginan_pribadi"),
      RENCANA_SETELAH_PULANG: v("rencana_setelah_pulang"), TUJUAN_KE_JEPANG: v("tujuan_ke_jepang"),
      STATUS_EKS_JEPANG: v("status_eks_jepang"), KONTAK_DARURAT_NAMA: v("kontak_darurat_nama"),
      KONTAK_DARURAT_HUBUNGAN: v("kontak_darurat_hubungan"), KONTAK_DARURAT_WA: v("kontak_darurat_wa"),
      KENALAN_DI_JEPANG_NAMA: v("kenalan_di_jepang_nama"), KENALAN_DI_JEPANG_HUBUNGAN: v("kenalan_di_jepang_hubungan"),
      KENALAN_DI_JEPANG_PEKERJAAN: v("kenalan_di_jepang_pekerjaan"), KENALAN_DI_JEPANG_USIA: v("kenalan_di_jepang_usia"),
      KENALAN_DI_JEPANG_ALAMAT: v("kenalan_di_jepang_alamat"), NO_PASPORT: v("no_paspor"),
      TGL_TERBIT_PASPORT: v("tgl_terbit_pasport"), EXP_PASPORT: v("exp_pasport"),
      KOTA_TERBIT_PASPORT: v("kota_terbit_pasport"), NO_COE: v("no_coe"),
      PAS_PHOTO: v("pas_photo"), JFT_URL: v("jft_url"), SSW_URL: v("ssw_url"),
      FILE_CV: v("file_cv"),
    };
    for (let i = 1; i <= 5; i++) {
...
async function handleSubmitMasterForm(payload, sessionToken) {
  ...
  try {
    ...
    (Array.isArray(d.pendidikan) ? d.pendidikan : []).slice(0, 5).forEach((p, i) => {
      ...
      if (p.nama_sekolah !== undefined) body["pendidikan_" + n + "_nama_sekolah"] = String(p.nama_sekolah);
    ...
    (Array.isArray(d.pekerjaan) ? d.pekerjaan : []).slice(0, 3).forEach((p, i) => {
      const n = i + 1;
      if (p.nama_perusahaan !== undefined) body["pekerjaan_" + n + "_nama_perusahaan"] = String(p.nama_perusahaan);
      if (p.jabatan !== undefined) body["pekerjaan_" + n + "_jabatan"] = String(p.jabatan);
      if (p.tahun_masuk !== undefined) body["pekerjaan_" + n + "_tahun_masuk"] = String(p.tahun_masuk);
      if (p.tahun_keluar !== undefined) body["pekerjaan_" + n + "_tahun_keluar"] = String(p.tahun_keluar);
  ...
  try {
    ...
    if (map) {
      ...
      if (map.pemberkasan) {
        ...
        const existing = Array.isArray(pRows) ? pRows.find((r) => supabase.normalizeWa(String(r.wa || "")) === want) : null;
        const pBody = { wa: wa, nama_lengkap: nama, tahap: 1, updated_at: new Date().toISOString(), [map.pemberkasan]: url };
        if (existing && existing.id !== undefined) {
  ...
  try {
    if (id && id !== "") {
      ...
      await supabase.supabaseJson("POST", "wa_templates", {
        body: { id: "WA" + Date.now(), nama, isi, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
        headers: { Prefer: "return=minimal" },
      });
    }
    return { success: true };
...
async function handleKirimTawaranMassal(payload, sessionToken) {
  ...
  try {
    ...
    try {
      ...
      const tpl = (Array.isArray(rows) ? rows : []).find((r) =>
        String(r.nama || "").toLowerCase().includes("grup") || String(r.nama || "").toLowerCase().includes("undang")
      );
    ...
    for (const c of cands) {
      ...
      let message =
        d.customMessage || templateIsi ||
        "Halo " + nama + "! Anda terpilih untuk Lowongan " + jobCode + ". Silakan bergabung ke grup resmi kami: " + linkGrup;
      if (templateIsi && !d.customMessage) {
        message = templateIsi.replace(/\{nama\}/g, nama).replace(/\{job\}/g, jobCode).replace(/\{link\}/g, linkGrup);
      }
      try {
  ...
  try {
    ...
    for (const item of items) {
      ...
      await supabase.supabaseJson("POST", "sys_config", {
        body: { config_type: type, config_value: item, is_active: true, created_at: new Date().toISOString() },
        headers: { Prefer: "return=minimal" },
      });
    }
    return { success: true };
...
async function handleGenerateFormBridge(payload) {
  ...
  const formUrl = siteBase() + "/apply-full.html?job=" + encodeURIComponent(code) + "&bidang=" + encodeURIComponent(bidang) + "&wa=&nama=";
...
async function handleGenerateAiFormBridge(payload) {
  ...
  const formUrl = siteBase() + "/ai_form.html?job=" + encodeURIComponent(code) + "&wa=" + encodeURIComponent(wa);
  return { qrUrl: qrUrl(formUrl), formUrl };
}
...
async function handleGetDriveLinkCandidates(payload, sessionToken) {
  ...
  try {
    ...
    const data = found.rows
      .filter((r) => /drive\.google/i.test(String(supabase.pick(r, ["folder_url", "folderUrl", "folder_id"]) || "")))
...
async function handleUploadDriveReplacement(payload, sessionToken) {
  ...
  try {
    ...
    const map = labelKey ? FILE_LABEL_COLUMNS[labelKey] : null;
    const found = await supabase.findCandidates();
    const c = found.rows.find((r) => String(supabase.pick(r, ["id_kandidat", "id"]) || "") === idKand);
