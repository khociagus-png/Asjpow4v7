...
| 2026-08-17 | khoci89 (user: "Deploy Ke netlivy", kirim token `nfp_…sb844` via chat) | `asjportal` | Deploy `693931b` (fix SW preview no-op + PWA auto-update + kontras light) via **`scripts/deploy-netlify.mjs`** (SKIP_INSTALL/SKIP_BUILD — build lokal sudah identik dengan repo). Deploy ID `6a8311c8ba99f1911dd51677`; hashing 328 file + 19 functions (CDN diff 0 baru — file sudah ter-upload oleh Freebuff deploy yang sempat putus; deploy ini membuat deploy baru yang valid). **Kunci: `sw.js` live kini precache `app-935b39d018.js`** (fix SHELL bundle lama `f9a83ca`) + VERSION `asj-portal-app-935b39d018-m886a44dc` + header `no-cache` — user HP tidak akan nyangkut versi lama lagi. Verifikasi live: homepage 200, bundle `app-935b39d018.js` 200, getAppData jobs=132 |
