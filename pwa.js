...
(function () {
  ...
  var IS_DEV_HOST = (typeof location !== 'undefined') &&
  ...
  if (IS_DEV_HOST && 'serviceWorker' in navigator) {
    ...
    window.addEventListener('pointerdown', function () { userInteracted = true; }, { once: true, passive: true });
    window.addEventListener('keydown', function () { userInteracted = true; }, { once: true });
  ...
  window.bersihkanDraftLamaBase64 = function () {
    try {
      ...
      keys.forEach(function (key) {
        ...
        if (!raw) return;
        var parsed = null;
        try { parsed = JSON.parse(raw); } catch (e) { /* rusak */ }
        ...
        var jftB = parsed.currentJftBase64, sswB = parsed.currentSswBase64;
        var jftF = parsed.currentJftFile, sswF = parsed.currentSswFile;
        var isOldFormat = (typeof jftB === 'string' && jftB.length > 0) ||
        ...
        if (isOldFormat) {
          ...
          var bersih = { chatHistory: parsed.chatHistory || [], latestCandidateData: parsed.latestCandidateData || {}, currentPhotoBase64: parsed.currentPhotoBase64 || '' };
