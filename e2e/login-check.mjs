...
// -------------------------------------------------------------
{
  ...
  const rendered = await waitFor(async () => (await page.locator("#public-table-body tr").count()) > 0);
...
// -------------------------------------------------------------
{
  ...
  const modalVisible = await waitFor(async () => await page.locator("#form-login-kandidat").isVisible());
  ...
  const bioRendered = await waitFor(async () =>
    (await page.locator("#prog-biodata-badge").innerText()).indexOf("?") === -1
  );
...
// -------------------------------------------------------------
{
  ...
  check("Menu hamburger admin aktif", await page.evaluate(() => !document.getElementById("mobile-nav-admin").classList.contains("hidden")));

  // Dashboard render (fungsi render yang baru di-refactor: jadwal, kandidat, dbjob)
  const jadwal = await waitFor(async () => (await page.locator("#admin-jadwal-body tr").count()) > 0);
  const kandidat = await waitFor(async () => (await page.locator("#admin-kandidat-body tr").count()) > 0);
