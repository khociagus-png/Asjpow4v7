  ...
  return page.evaluate(() => {
    ...
    document.querySelectorAll("img").forEach((img) => {
      ...
      if (img.complete && img.naturalWidth === 0 && shown) {
        out.push({ src: img.src.slice(0, 150), visible, cls: (img.className || "").toString().slice(0, 45) });
      }
    });
    return out;
  });
...
// -------------------------------------------------------------
{
  ...
  check("Foto/aset publik semua termuat", failed.length === 0 && failedReqs.length === 0,
...
// -------------------------------------------------------------
{
  ...
  } catch (e) { console.log("  (bukaModalCvMini error:", String(e).slice(0, 80), ")"); }
  ...
  console.log("  request gagal (gambar):", failedReqs.length);
  failedReqs.forEach((u) => console.log("    !", u));
  check("Tidak ada foto gagal di dashboard kandidat", failed.length === 0 && failedReqs.length === 0,
    JSON.stringify([...(failed.slice(0, 2)), ...(failedReqs.slice(0, 2))]));
...
// -------------------------------------------------------------
{
  ...
  const opened = await page.evaluate(() => {
    ...
    const names = ["bukaModalKandidatAdmin", "bukaDetailKandidat", "adminBukaKandidat", "bukaAdminKandidat", "bukaModalKandidat"];
  ...
  if (opened) {
    ...
    if (await btn.count()) { await btn.click(); await new Promise((r) => setTimeout(r, 1800)); }
  }
  ...
  console.log("  request gagal (gambar):", failedReqs.length);
  failedReqs.forEach((u) => console.log("    !", u));
  check("Tidak ada foto gagal di modal admin", failed.length === 0 && failedReqs.length === 0,
    JSON.stringify([...(failed.slice(0, 2)), ...(failedReqs.slice(0, 2))]));
