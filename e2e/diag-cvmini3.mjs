...
const getPath = (o, ...ks) => { for (const k of ks) { if (o && o[k] !== undefined && o[k] !== null && o[k] !== '') return o[k]; } return '-'; };
console.log('SEBELUM  jft:', getPath(b, 'jft', 'nilai'), '| bidangssw:', getPath(b, 'bidangssw', 'ssw'), '| pend1:', getPath(b, 'pendidikan_1_tingkat'), '| foto:', getPath(b, 'pas_photo'));

// Simpan via payload PERSIS CV Mini (prosesSimpanCvMini)
const res = await handleAction('simpanUpdateMaster', [{
  wa: waNorm,
  nama: 'AGUS KHOCI',
  gender: 'LAKI-LAKI',
  usia: '24',
...
console.log('SESUDAH  jft:', getPath(a, 'jft', 'nilai'), '| bidangssw:', getPath(a, 'bidangssw', 'ssw'), '| pend1:', getPath(a, 'pendidikan_1_tingkat'));
...
await handleAction('simpanUpdateMaster', [{
  wa: waNorm, nama: 'AGUS KHOCI', gender: 'LAKI-LAKI', usia: '24', tb: '165', bb: '57',
  pendidikan: String(getPath(b, 'pendidikan_1_tingkat') !== '-' ? getPath(b, 'pendidikan_1_tingkat') : 'SMAN 1'),
  jft_text: getPath(b, 'jft', 'nilai') !== '-' ? String(getPath(b, 'jft', 'nilai')) : '',
  ssw_text: getPath(b, 'bidangssw', 'ssw') !== '-' ? String(getPath(b, 'bidangssw', 'ssw')) : '',
}], tok);
